...
def plot_data(data):
    ...
    eeg_data = data.iloc[:, 1:9]
    timestamps = data.iloc[:, 9]
    timestamps = (timestamps - timestamps.iloc[0]) / 1e6
    ...
    for i, name in enumerate(channel_names):
        plt.plot(timestamps, eeg_data.iloc[:, i], label=name, color=color_map[name], linewidth=0.5, alpha=0.8)
...
def run(read_eeg_data, read_eeg_data_brainflow, acquisition_method, socketio, running, get_file_path, wait_for_file):
    print("Starting BrainFlow data plotting experiment")
    socketio.emit('experiment_update', {
        "message": "Starting BrainFlow data plotting experiment",
    ...
    try:
        ...
        stats = {}
        for i, name in enumerate(channel_names):
            channel_data = eeg_data.iloc[:, i+1]
            stats[name] = {
        ...
        socketio.emit('experiment_update', {
            ...
            "plotTitle": "EEG Waveforms from BrainFlow",
        ...
        print("BrainFlow data plotting experiment completed")
        socketio.emit('experiment_update', {"message": "BrainFlow data plotting experiment completed"})
